...
class MovieLens():
    def __init__(self, host):
        ...
        # create indexes
        assert self.graph.run("CREATE INDEX ON :Movie(movieId)").data() == []
        assert self.graph.run("CREATE INDEX ON :Movie(title)").data() == []
    ...
    def recommendation(self, movieIds, category=None, rating=3.5, count=4, limit=5):
        return self.graph.run(f"""MATCH (m:Movie)-[r1]-(u)-[r]-(m2{':`' + category + '`' if category else ''})
            ...
            ORDER BY cnt DESC, rate DESC LIMIT {limit}""").data()
